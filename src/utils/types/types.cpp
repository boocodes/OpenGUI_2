...
UI_FONT::UI_FONT(int x_pos, int y_pos, glm::vec3 color, float scale, std::string font_name, std::string text)
{
	this->x_pos = x_pos;
	this->y_pos = y_pos;
	this->color = color;
...
void UI_FONT::draw()
{
    this->font.draw_text(this->text, static_cast<float>(x_pos), static_cast<float>(y_pos), this->scale, this->color);
}
...
void Font::draw_text(std::string text_display, float x_pos, float y_pos, float scale, glm::vec3 color)
{
	...
	for (c = text_display.begin(); c != text_display.end(); c++)
	{
		...
		x_pos += (ch.Advance >> 6) * scale; // битовый сдвиг на 6 разрядов, чтобы получить значение в пикселях (2^6 = 64 (разделите количество 1/64-х пикселей на 64, чтобы получить количество пикселей))
	}
	glBindVertexArray(0);
